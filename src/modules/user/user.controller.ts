...
export class UserController {
  ...
  async createUser(req: AuthRequest, res: Response) {
    try {
      ...
      return res
        .status(500)
        .json({ message: "Não foi possível criar a conta. Por favor, tente novamente." });
    }
  }
  ...
  async getUserById(req: AuthRequest, res: Response) {
    try {
      ...
      if (!user) {
        return res
          .status(404)
          .json({ message: "Não encontrámos nenhum utilizador com esse identificador." });
      }
      if ("status" in user && user.status !== 200) {
        return res.status(user.status).json({ message: user.message });
      }
  ...
  async getUserMe(req: AuthRequest, res: Response) {
    try {
      ...
      return res
        .status(500)
        .json({ message: "Não foi possível carregar o seu perfil. Por favor, tente novamente." });
    }
  }
  ...
  async updateUser(req: AuthRequest, res: Response) {
    try {
      ...
      if (!user) {
        return res
          .status(404)
          .json({ message: "Não encontrámos o utilizador que pretende atualizar." });
      }
      return res.status(200).json(user);
    } catch (error) {
      console.error(error);
      return res
        .status(500)
        .json({ message: "Não foi possível guardar as alterações. Por favor, tente novamente." });
    }
  }
  ...
  async deleteUser(req: AuthRequest, res: Response) {
    try {
      ...
      if (!result) {
        return res
          .status(404)
          .json({ message: "Não encontrámos o utilizador que pretende remover." });
      }
