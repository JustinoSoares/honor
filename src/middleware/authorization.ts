...
export const authentication = async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  if (!authHeader) {
    return res
      .status(401)
      .json({ message: "É necessário fazer login para aceder a esta funcionalidade." });
  }
  ...
  try {
    ...
    return res
      .status(401)
      .json({ message: "A sua sessão expirou ou é inválida. Por favor, faça login novamente." });
  }
};

export const authenticationAdmin = async (req: AuthRequest, res: Response, next: NextFunction) => {
  const authHeader = req.headers.authorization;
  if (!authHeader) {
    return res
      .status(401)
